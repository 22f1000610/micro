...
This version splits Chapter 5 into two chapters at "Duality".
"""
...
def parse_subtopics(html_file):
    ...
    # Flag to track if we've switched to new chapter
    switched_to_new_chapter = False
        ...
        if element.name == 'h1':
            ...
            if current_subtopic:
                structure.append({
                    ...
                    'subtopic_title': current_subtopic.get_text().strip() if hasattr(current_subtopic, 'get_text') else current_subtopic.get('text', ''),
            ...
            # Check if we need to split Chapter 5 at "Duality"
            h2_text = element.get_text().strip()
            
            # If we're in Chapter 5 and hit "Duality", create new chapter
            if current_chapter and current_chapter['title'] == 'Chapter 5 - COST FUNCTION' and h2_text == 'Duality' and not switched_to_new_chapter:
                # Save the current subtopic first
                if current_subtopic:
                ...
                current_chapter = {
                    'id': 'chapter-6-duality-welfare',
                    'title': 'Chapter 6 - Duality, Welfare Economics and other topics'
                }
                switched_to_new_chapter = True
                current_subtopic = element
                subtopic_content = []
                
            else:
                # Normal h2 processing - save previous subtopic
